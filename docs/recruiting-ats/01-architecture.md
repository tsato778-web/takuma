...
| 認証 | **Auth.js v5（共通管理者アカウント1つ）** | 本部3名で1つのIDとパスワードを共有する運用（2026-08-17 決定）。個人別アカウント・権限管理は作らず、業務機能の実装に時間を充てる。将来は providers を追加するだけで個人別ログインへ移行できる |
...
**設置場所（2026-08-17 決定）**：本システムは既存の NAORU アカデミーダッシュボード（本リポジトリの `index.html` / `api/kpi.js`）とは別プロダクト（別ドメイン・別デプロイ・別 DB）だが、当面は**同一リポジトリの `apps/recruiting/` 配下**で開発する。

- 既存ダッシュボードのファイル・ビルド・デプロイには一切影響しない（Vercel 側で **Root Directory を `apps/recruiting` に指定した別プロジェクト**として作成する）。
- 依存関係・ロックファイル・CI はアプリ配下で独立しているため、将来的に単独リポジトリへ切り出す場合もディレクトリごと移動するだけで済む。
...
| 認証 | 共通の管理者アカウント1つ（ID＋パスワード）。パスワードは scrypt ハッシュで環境変数に保管し、照合は timingSafeEqual。連続失敗時は一時的にログインを受け付けない |
| 権限 | **現時点では権限管理を作らない**（ログインできる＝全機能利用可）。将来に備え `users.role` / `users.canLogin` と操作ログの構造だけ用意しておく |
...
0. **認証は共通アカウント1つ**（2026-08-17 決定）：本部3名で ID とパスワードを共有する。個人別ログイン・権限管理・院長/オーナー別ログインは現時点では実装しない。監査ログの「誰が」は共通アカウント単位の記録になる点だけ留意する。
